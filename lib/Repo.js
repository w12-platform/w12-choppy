...
class Repo {
    constructor(version) {
        ...
        this.archivePath = path.resolve(process.cwd(), './.archives/', version + '.zip');
    ...
    async downloadArchive() {
        ...
        if (!await utils.exists(path.dirname(this.archivePath))) {
            await utils.mkdir(path.dirname(this.archivePath));
        }
    ...
    async unzipArchive() {
        ...
        if (!await utils.exists(path.dirname(this.containerPath))) {
            await utils.mkdir(path.dirname(this.containerPath));
        }
    ...
    async bootstrap() {
        ...
        await execa.shell('npm ci', { cwd: this.repoPath });
    }
    ...
    async execScript(script) {
        ...
        const proc = execa.shell('truffle exec -c ' + script.path, { cwd: this.repoPath });
    ...
    async install() {
        ...
        console.log(this.version, ': installed');
    }
}
